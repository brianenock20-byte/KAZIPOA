    ...
    <AuthShell {...modeCopy}>
      ...
      {mode === "verify" ? (
        ...
        <form className="custom-auth-form" onSubmit={handleSubmit} noValidate>
          ...
          {mode !== "forgot" && <PasswordField id="password" label={mode === "reset" ? "New password" : "Password"} value={password} onChange={value => { setPassword(value); setPasswordTouched(true); }} autoComplete={mode === "reset" || mode === "register" ? "new-password" : "current-password"} hint="Use 12+ characters. Long passphrases are easier to remember and stronger." />}
