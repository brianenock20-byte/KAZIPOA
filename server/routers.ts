...
import { joinWaitlist, countWaitlistSignups, listWaitlistSignups } from "./earlyAccess";
...
export const appRouter = router({
  ...
  waitlist: router({
    join: publicProcedure.input(z.object({ name: z.string().trim().min(2).max(180), email: z.string().email().max(320), phone: z.string().trim().max(32).optional(), intent: z.enum(["seeker", "employer", "not_sure"]) })).mutation(({ input }) => joinWaitlist(input)),
    count: adminProcedure.query(() => countWaitlistSignups()),
    list: adminProcedure.query(() => listWaitlistSignups()),
  }),
